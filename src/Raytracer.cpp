...
void Raytracer::initializeSkybox()
{
    ...
    for (unsigned int i = 0; i < 6; i++)
    {
        ...
        shader->skybox->setFaceTexture(data, i, width, height);
    }
}
void Raytracer::initializeScene()
{
    ...
    auto sphere1 = new Cube{ glm::vec3{-4, 2, -5}, 6.6f, {{0 * DEG_TO_RAD, 0 * DEG_TO_RAD, 14 * DEG_TO_RAD}} };
    ...
    plane3->material = new Material(Color::white(), true, tex, 1, 0.5, 100, 0);
    plane4->material = whiteWallMat;
    plane5->material = whiteWallMat;

    sphere1->material = new Material(Color::gold(), true, tex, 1, 0, 2000, 0.3f);
