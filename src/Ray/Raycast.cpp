...
Color Raycast::castRay(Ray ray, int bounce)
{
	...
	for (int i = 0; i < bounce; ++i)
	{
		...
		v += colorImpact * (1 - ray.reflection) * ray.color * getIlluminationAtPoint(ray.interPoint, ray.surfaceNormal);
		colorImpact *= ray.reflection;
		if (colorImpact <= 1e-6)
			break;
//TODO specular, diffuse, random rays
		auto dir = ray.dir - (2 * dot(ray.dir, ray.surfaceNormal) * ray.surfaceNormal);//* ((float)(rand()%1000)/1000);
