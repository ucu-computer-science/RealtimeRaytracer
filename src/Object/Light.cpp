...
PointLight::PointLight(glm::vec3 pos, Color color, float distance,
                       float intensity)
        : Object(pos, {{0, 0, 0}}), distance(distance), intensity(intensity),
...
void PointLight::getIlluminationAtPoint(const Ray &ray, Color &inColor,
    ...
    inSpecular +=
            distanceImpact * std::pow(std::max(dot(H, ray.surfaceNormal), 0.0f), ray.material->specular_degree) * color;
}
...
Light::Light(glm::vec3 pos, Color color, float distance, float intensity,
             glm::vec3 size, glm::vec3 pointSize)
...
//            if(lightFacingAtPoint == 1) exit(1);
    ...
    inSpecular +=
            std::pow(std::max(dot(H, ray.surfaceNormal), 0.0f), ray.material->specular_degree) * color;
//    auto R = normalize(2 * lightFacingAtPoint * ray.surfaceNormal - (pos));
////    if(dot(R, -ray.dir) > 1) exit(1);
//
